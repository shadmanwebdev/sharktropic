...
class ContactMessage extends Db {
    ...
    public function get_messages() {
        $stmt = $this->con->prepare("SELECT * FROM messages ORDER BY id DESC");
        $stmt->execute();
        $result = $stmt->get_result();
        $data = $result->fetch_all(MYSQLI_ASSOC);
    ...
    public function create() {
        ...
        $stmt = $this->con->prepare("INSERT INTO messages (firstname, lastname, email, msg_subject, msg, created_at) VALUES (?, ?, ?, ?, ?, ?)");
        ...
        if($stmt->execute()) {
            ...
            if (isset($_FILES['message_file']) && $_FILES['message_file']['error'] === UPLOAD_ERR_OK) {
                ...
                $tempFilePath = $_FILES['message_file']['tmp_name'];
            
                // Define the directory where you want to save the image
                $imageDirectory = '../images/contact_messages/'; // Update this path to your desired directory
            ...
            // Insert Image
            $stmt2 = $this->con->prepare("INSERT INTO message_filename (message_id, flname) VALUES (?, ?)");
            $stmt2->bind_param("is", $message_id, $new_photo);
            if($stmt2->execute()) {
                $stmt2->close();
            }
