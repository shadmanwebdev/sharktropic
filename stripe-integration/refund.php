<?php
    ...
    if(isset($_GET['refund'])) {
        ...
        $refund_response = $sp->refund($_GET['id']);

        $refund_status = $refund_response['status'];
        if ($refund_status == 'succeeded') {
            $sp->update_payment_status($_GET['id'], 'refunded');
            $payment = $sp->get_payment_by_checkout_session_id($_GET['id']);

            $booking_id = $payment['booking_id'];

            $bk = new Booking;
            $bk->update_booking_status($booking_id, $status);
        }
    }
...
<body>
    ...
    <?php
        if ($refund_status == 'succeeded') {
            $status_text = "Success!";
            $status_msg = "Payment was refunded";
            ...
            $status_msg = "Refund attempt failed";
        }
    ?>
        ...
        <div class='popup-inner-div'>
            ...
            <div class='popup-subtitle'>
                ...
                <div class='back' style='margin-top: 20px;'>
                    <a href='../admin/bookings'>Back to Admin Panel</a>
                </div>
            </div>
        </div>
    </div>
