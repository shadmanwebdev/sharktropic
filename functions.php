<?php
    ...
    */ 
    ...
    function date_now($tz='Europe/London') {
        $now = new DateTime("now", new DateTimeZone($tz) );
        $date = $now->format('Y-m-d');
        return $date;
    }
    function datetime_now($tz='Europe/London') {
        $now = new DateTime("now", new DateTimeZone($tz) );
        $datetime = $now->format('Y-m-d H:i:s');
        return $datetime;
    }
    function datetime_mjy($dt, $tz='Europe/London') {
        $new_dt = new DateTime($dt, new DateTimeZone('Europe/London') );
        return $new_dt->format("M j, Y h:i A");
    }
    /*
    function elapsed($dt_from, $tz='Europe/London') {
    ...
    */
    ...
    function elapsed_check($dt_from, $tz='Europe/London') {
        $created_at = new DateTime($dt_from, new DateTimeZone($tz) );
        $date = new DateTime("now", new DateTimeZone($tz) );
    ...
    function convert_date($d, $f, $zone='Europe/London') {
    ...
    function datetime_array($datetime_str) { // '2023-04-06T22:48:00.000Z'
    ...
    */ 
    function photoOrDefault($user_photo, $admin=null) {
        ...
        if ($_SERVER['SERVER_NAME'] == 'localhost') {
            $root = 'http://'.$_SERVER['SERVER_NAME'].'/'.'Spotzy';
        } else {
    ...
    */
    ...
    function limitWords($text, $maxLength, $returnLength) {
        ...
        if ($wordCount > $maxLength) {
            ...
            $limitedWords = array_slice($words, 0, $returnLength);
